...
const Message = ({ messages, currentUser, messagesContainerRef }) => {
  ...
  return (
    <Paper className='chat-messages' ref={messagesContainerRef} elevation={3} style={{ flex: 1, padding: 16, overflowY: 'auto' }}>
      {canLoadMore && (
      ...
      {messages.slice(-visibleMessages).map((message, index) => {
        const isSameSender = lastSender === message.sender._id;
